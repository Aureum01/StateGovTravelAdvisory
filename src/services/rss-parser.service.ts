...
export class RSSParserService {
  ...
  private readonly RSS_URL = 'https://travel.state.gov/_res/rss/TAsTWs.xml';
   ...
   */
  async fetchTravelAdvisories(): Promise<TravelAdvisoryFeed> {
    try {
      const feed = await this.parser.parseURL(this.RSS_URL) as any;

      // The rss-parser library returns a flat structure
      // We need to wrap it in the expected TravelAdvisoryFeed format
      const result: TravelAdvisoryFeed = {
        rss: {
          channel: {
   ...
   */
  private processAdvisoryItem(item: TravelAdvisoryItem): ProcessedTravelAdvisory {
   ...
   */
  private parseCategories(categories: any[]): { threatLevel: string; countryTag: string; keywords: string[] } {
    const result = {
      threatLevel: '',
      countryTag: '',
      keywords: [] as string[]
   ...
   */
  private extractCountryInfo(title: string): { country: string; countryCode: string } {
   ...
   */
  private extractThreatLevel(level: string): { level: string; levelNumber: number } {
   ...
   */
  private parseDescription(description: string): { restrictions: string[]; recommendations: string[] } {
